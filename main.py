    ...
    with st.expander('Free Response Problem'):
        ...
        if submitted:
            response = openai.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert in the field of " + st.session_state.topic + " and are designed to provide feedback on user answers without giving away the actual answer. Do not acknowledge or greet. Address the user's answer directly using second person."},
                    {"role": "user", "content": "Provide feedback without directly or indirectly giving away the answer: " + user_answer + " for the question: " + problem['Problem_statement'] + "using the correct answer: " + problem['Correct_answer']},
                ],
                seed=138,
                temperature=0.1,
            )    
...
if topic != st.session_state.topic and topic is not None and topic != "":
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a course textbook writing expert designed to output in JSON following the format: " + example},
        ...
        with chapter_tab:
            for section in chapter['sections']:
                ...
                section_content = openai.chat.completions.create(
                    model=st.session_state["openai_model"],
                    messages=[
                        {"role": "system", "content": "You are a course textbook content generation machine designed to output in markdown(surround inline latex with $..$ and display latex with $$..$$). Do not acknowledge or greet. Output the content only. Expand on information where appropriate."},
                        {"role": "user", "content": "Generate the content of the section (do not include section title in reponse): " + section + " in the" + st.session_state.topic + " textbook."},
                    ],
                    temperature=0.4,
                )
                ...
                problem_content = openai.chat.completions.create(
                    ...
                    response_format={ 'type': 'json_object'},
                    messages=[
                        {"role": "system", "content": "You are a course textbook problem generation machine designed to output in JSON in the format: " + output_example + "Follow the problem type formatting exactly and using markdown(surround any inline latex math expressions with $..$ and display latex math expressions with $$..$$) for problem_statement field."},
                        {"role": "user", "content": "Generate exactly 2 problems of random types for the section: " + section + "referencing the section content" + raw_content},
                    ],
                    seed = 138,
                    temperature=0.4,
                )
