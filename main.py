...
def generate_content(selected_chapters):
    ...
    # print(f"\nstart here -- {st.session_state.chapters} --\n")
    for chapter in selected_chapters:
        for i, section in enumerate(chapter["sections"]):
            section_content = generate_section_content(section)
            # print(f"\ncontent call -- {st.session_state.chapters} --\n")
            api_calls += 1
            ...
            problems = generate_section_problems(section, section_content)
            # print(f"\nproblem call -- {st.session_state.chapters} --\n")
            api_calls += 1
            print(f"{api_calls}/{total_calls}")
            my_bar.progress(api_calls / total_calls, text="Generating Content")
            ...
            # print(f"\nassignment -- {st.session_state.chapters} --\n")
...
@st.cache_data(show_spinner=False)
def generate_section_problems(section, section_content):
    output_example = r"""{
        Problems: [
            {
...
def content_selection(selected_chapters, section_selections):
    ...
    with st.expander(
        ...
        with st.form(key="content_selection_form"):
            select_all_chapters = [
                "Select All Of " + chapter["chapter_label"]
                for chapter in st.session_state.chapters
            ]
            select_all_chapters.insert(0, "Select All Chapters")
            select_all = st.multiselect(
                ":rainbow[Select By Chapter:]",
                select_all_chapters,
            ...
            for i, chapter in enumerate(st.session_state.chapters):
                ...
                if len(st.session_state.selected_chapters) > 0:
                    default_selected = [
                        section["heading"]
                        for section in st.session_state.selected_chapters[i]["sections"]
                    ]
                # print(default_selected)
                # print(chapter["sections"])
            ...
            submitted = col2.form_submit_button(
                "Submit", on_click=close_expander, use_container_width=True
            )

    if submitted:
        selected_chapters = []
        # st.code(section_selections)
        if "Select All Chapters" in select_all:
            selected_chapters = st.session_state.chapters.copy()
        else:
            for i, chapter in enumerate(st.session_state.chapters):
                if "Select All Of " + chapter["chapter_label"] in select_all:
